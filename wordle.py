...
def test_guess_word(): #returns false if the code returns the wrong thing, true if it returns the right thing
    board = wordle_board
    if board.guess_word(board,"owen") == True or board.guess_word(board,"obingus") == True: # if the function returns true for an ineligible word
        ...
        board.testing_set_winning_word(board,"abbcd") # sets the winning word to johns
        if board.guess_word(board,"abcbc") == False: # if it says jhihy is not allowed
            return False
        else:
